...
- `@page { size: letter; margin: 0; }` CSS for print layout
- Each page as a `div` with `8.5in x 11in` dimensions and `page-break-after: always`
- Clean, professional design: navy headers, white background, data tables with alternating row shading
- Embed charts as `<img>` tags from matplotlib/plotly PNGs
- Export via Puppeteer `page.pdf()` with `printBackground: true`
